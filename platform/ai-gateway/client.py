...
class NIMClient:
    ...
    async def generate(self, model: str, messages: List[Dict[str, str]], temperature: float = 0.2, max_tokens: int = 1024) -> str:
        ...
        from platform.ai_gateway.main import nvidia_api_key_ctx, ai_gateway_mode_ctx
    ...
    def _mock_generation(self, model: str, messages: List[Dict[str, str]]) -> str:
        ...
        # Convert prompt/messages to lowercase for simple classification
        full_content = "\n".join([m["content"] for m in messages]).lower()
        
        # Scenario 1: JD Analysis & gap extraction
        if "analyze" in full_content or "job description" in full_content:
            return """
            {
              "extracted_skills": ["Python", "FastAPI", "PostgreSQL", "React", "Docker", "Kubernetes", "Next.js"],
              "keywords": ["Cloud Architecture", "Next.js", "CI/CD Pipeline", "Microservices", "Row-Level Security"],
              "gap_analysis": [
        ...
        # Scenario 2: Resume bullet optimization
        elif "optimize" in full_content or "tailor" in full_content or "bullet" in full_content:
            # We must output optimized text sections that incorporate the selected evidence.
            # Let's return a structured JSON representing optimized bullets or a general response.
            return """
            [Optimized Section: Professional Experience]
        ...
        # Scenario 3: Interview Question Generation
        ...
        # Scenario 4: Interview Coach Feedback
        elif "response" in full_content and ("evaluate" in full_content or "coach" in full_content):
            return """
            {
              "coaching_tips": "Your explanation of PostgreSQL Row-Level Security is structurally sound. To improve, mention specific query benchmarks (e.g. explain plans) and security audits you conducted. Be sure to link it explicitly to the project where you isolated multi-tenant data.",
              "next_question": "How do you handle background execution in a FastAPI system, and how do you monitor Celery tasks?",
              "completed": false
            }
            """
        
        # Scenario 5: Interview Coaching final report
        elif "report" in full_content or "readiness" in full_content:
            return """
            {
              "readiness_score": 86.5,
              "key_strengths": [
                "Detailed technical explanation of PostgreSQL RLS policies",
                "Familiarity with FastAPI backend routing and async capabilities",
                "Strong grasp of multi-tenant security architecture"
              ],
              "improvement_areas": [
                "Quantify project outcomes (e.g., performance improvements, query latency reductions)",
                "Describe error handling procedures and Celery worker health checks"
              ],
              "transcript": []
            }
            """
